...
| 步骤 | 函数 / 模块 | 说明 |
|------|-------------|------|
| Step 1 | `extract_text_with_page_numbers` | 使用 PyPDF2 逐页提取 PDF 文本，为每个字符记录对应页码 |
...
| Step 4 | `FAISS.from_texts` | 构建 FAISS 向量索引，并通过众数统计关联每个 chunk 的页码 |
| Step 5 | `save_local` + pickle | 将向量库与页码映射持久化到 `vector_db/` 目录 |
| Step 6 | `similarity_search` | 对用户问题进行语义检索，召回 Top-10 最相关文本块 |
| Step 7 | `Tongyi.invoke` | 将检索结果拼接为上下文，手动构建 Prompt 送入大模型生成回答 |
...
- 通过 `langchain_text_splitters` 独立包引入（LangChain 新版 API）
- 按 `\n\n`、`\n`、`.`、空格等分隔符递归切分，尽量保持语义完整
...
- 采用**手动 Prompt 拼接**方式（`llm.invoke`），替代旧版 `load_qa_chain`，更直观、可控

### 页码映射：字符级众数统计 + pickle

- 为 PDF 每个字符记录页码，分块后取块内页码的**众数**作为该 chunk 的来源页
- 比模糊文本匹配更准确，跨页 chunk 也能合理归属
- 与向量库一同保存为 `page_info.pkl`，加载时恢复

### 路径处理：基于 `__file__` 的绝对路径

- 使用脚本所在目录定位 PDF 和 `vector_db`，从任意工作目录运行均可
...
similarity_search（Top-10）
    │
    ▼
手动拼接 Prompt + Tongyi（deepseek-v3）
    │
    ▼
生成回答 + 来源页码
```
...
└── vector_db/                # 运行后生成的向量库（可复用，已 gitignore）
    ├── index.faiss
    ├── index.pkl
    └── page_info.pkl
```
...
pip install faiss-cpu dashscope "numpy<2"
```
...
| 包名 | 版本 | 用途 |
|------|------|------|
| langchain_community | 0.4.1 | DashScope Embeddings、FAISS、通义 LLM |
| langchain_text_splitters | 1.1.0 | 文本分割（LangChain 新版独立包） |
| PyPDF2 | 3.0.1 | PDF 文本提取 |
| faiss-cpu | 1.7.4 | 向量索引与相似度搜索（需额外安装） |
| dashscope | 最新 | 阿里云百炼 API 底层 SDK（需额外安装） |
...
| 对比项 | 01_rag 版本 | 本版本（02_rag-technology） |
|--------|-------------|----------------------------|
| 文本分割 API | `langchain.text_splitter` | `langchain_text_splitters`（新版独立包） |
| 页码映射 | 行级模糊匹配 | 字符级映射 + 众数统计 |
| 问答方式 | `load_qa_chain`（stuff 链） | 手动拼接 Prompt + `llm.invoke` |
| 检索数量 | Top-2 | Top-10 |
| LangChain 版本 | 0.3.25 | community 0.4.1 + text_splitters 1.1.0 |

本版本在页码溯源精度和 Prompt 可控性上有所改进，并采用了 LangChain 较新的 API 结构。
